...
clang++ -Wall -O2 -fopenmp -stdlib=libc++ -L/usr/local/lib -L/Library/Frameworks/R.framework/Resources/lib -o main main.cpp
```
...
Reading the input tree, data and PMM parameters from the file test-input.txt...
Calculating the a, b, c at the root for sigma=0.5 and sigmae=1
a=-0.432535, b=0.630438, c=-8.21348
OpenMP-version: 201107
Good bye!
```

If the OpenMP-version is equal to 0, then, most likely your compiler does not support OpenMP and the header "omp.h" was not available during compilation. If the OpenMP-version was a different number, then your compiler has an other Open MP version in it. If there have been other differences, e.g. the reported values for a, b and c are different, then there seems to be an unknown bug, which I am not aware of. Please, report this in the [SPLITT issues list](https://github.com/venelin/SPLITT/issues).

# Next steps

The next step is to understand what the main.cpp file, which we just compiled, actually does. This is explained in the article [Writing a traversal specification](https://venelin.github.io/SPLITT/articles/SPLITTTraversalSpecification.html).
